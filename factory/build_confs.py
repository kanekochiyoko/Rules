...
str_module_head = open("template/sr_module_head.txt", "r", encoding="utf-8").read()
...
values["gfwlist"] = getRulesStringFromFile(
    "resultant/gfw.list", "Proxy"
...
module_names = {"sr_top500_banlist", "sr_top500_banlist_ad"}

for module_name in module_names:
    file_template = open(
        "template/" + module_name + "_module" + ".txt", "r", encoding="utf-8"
    )
    template = file_template.read()

    template = str_module_head + template

    file_output = open("../" + module_name + ".module", "w", encoding="utf-8")
    ...
    template = template.replace("DOMAIN-SUFFIX,duckdns.org,Proxy", "")
    file_output.write(template)
